...
<?php
		   ...
		   $query = $conn->prepare("SELECT id AS level_id FROM levels ORDER BY level_name ASC");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT id AS level_id,level_name FROM levels ORDER BY id ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
        ...
        <table width="100%" class="table table-striped"  id="datatable-fixedcolumns" >
            ...
            ?>
            <tr>  
                <td><?php echo titleCase($row['level_id']); ?></td>
				<td><?php echo titleCase($row['level_name']); ?></td>
                <td></td>
            </tr>
            <?php
            }
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete PO Item
    url: '../process/edit_level.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[1, 'LevelName']]
    }
    , buttons: {
        edit: {
