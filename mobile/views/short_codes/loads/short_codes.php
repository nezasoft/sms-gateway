...
include("../../../../connect/connect.php");
          ...
          if($_SESSION['FON_G_COMPANY_ID']==2 && $_SESSION['FON_G_LEVEL_ID']==4){
           ...
           $where ="WHERE csc.company_id LIKE '%%'";
           }else{
           $company_id = sanitize_string($_GET['company']);
           $where ="WHERE csc.company_id=".$company_id."";
           }
		...
		return $setPaginate;
         ...
         $pageLimit  = ($page * $setLimit) - $setLimit;
			$query = $conn->prepare("SELECT csc.id AS company_short_code_id,c.id AS company_id,csc.short_code_id,csc.response_message,csc.keyword,csc.endpoint,csc.correlator,csc.date_created,sc.short_code,sc.short_code_description,sc.service_id, p.product_name,sc.product_id, c.company_name, us.fname, us.lname FROM company_short_codes AS csc 
                                   LEFT JOIN short_codes AS sc ON sc.id = csc.short_code_id
                                   LEFT JOIN products AS p ON p.id = sc.product_id
                                   LEFT JOIN company AS c ON c.id = csc.company_id
       ...
       <th>Short Code</th>
        ...
        foreach($rows as $row){
            ...
            <td>          
            <a href="?my_account&action=view_messages&short_code=<?php echo $row['short_code_id']; ?>&company=<?php echo $row['company_id']; ?>&service_id=<?php echo $row['service_id']; ?>"  id="show_inbox_<?php  echo $row['company_short_code_id'];?>" class="btn btn-success widget-btn-2 btn-xs btn-custon-rounded-three"><i class="fa fa-envelope edu-informatio" aria-hidden="true"></i> Messages</a>
            <?php
            if($company_id=='2' && $_SESSION['FON_G_LEVEL_ID']=='4'){
            ?>
            ...
            <a href="#" id="get_messages_<?php  echo $row['company_short_code_id'];?>" class="btn btn-warning widget-btn-2 btn-xs btn-custon-rounded-three"><i class="fa fa-exchange" aria-hidden="true"></i> Fetch</a>
            <a href="#" id="deactivate_account_<?php  echo $row['company_short_code_id'];?>" class="btn btn-danger widget-btn-2 btn-xs btn-custon-rounded-three"><i class="fa fa-power-off" aria-hidden="true"></i> Deactivate</a>
            <a href="#" id="activate_account_<?php  echo $row['company_short_code_id'];?>" class="btn btn-success widget-btn-2 btn-xs btn-custon-rounded-three"><i class="fa fa-power-off" aria-hidden="true"></i> Activate</a>
            
            <span id="loader_<?php  echo $row['company_short_code_id'];?>"><img src="../img/loader.gif" width="30" height="30" />Requesting...</span> 
            </td>
            ...
            company = <?php echo $row['company_id']; ?>;
          ...
          $('#other_content').empty();
 				  $('#page_content').empty();
              $("#wait").show();
              $('#page_content').load('../views/short_codes/messages.html?company='+company+'&short_code='+short_code+'&service_id='+service_id,function(responseTxt, statusTxt, jqXHR){
            ...
            var myData = 'service_id=' + service_id + '&correlator=' + correlator + '&product_id='+product_id ; 
            ...
            $("#activate_account_<?php  echo $row['company_short_code_id'];?>").click(function(e){
            e.preventDefault();
            $("#response_<?php  echo $row['company_short_code_id'];?>").empty();
            $("#loader_<?php  echo $row['company_short_code_id'];?>").show();
            service_id = "<?php echo $row['service_id']; ?>";
            ...
            keyword = "<?php echo $row['keyword']; ?>";
            endpoint = "<?php echo $row['endpoint']; ?>";
            short_code = "<?php echo $row['short_code']; ?>";
            var myData = 'service_id=' + service_id + '&correlator=' + correlator + '&product_id='+product_id + '&keyword='+keyword  + '&endpoint='+endpoint + '&short_code='+short_code; 
            //alert(myData);
	         jQuery.ajax({
                type: "POST", // Post / Get method
                url: "../process/startSmsNotification.php", //Where form data is sent on submission
                dataType:"text", // Data type, HTML, json etc.
                data:myData, //Form variables
                success:function(response){
                $("#response_<?php  echo $row['company_short_code_id'];?>").html(response);
            ...
            //Get Messages
            $("#get_messages_<?php  echo $row['company_short_code_id'];?>").click(function(e){
            e.preventDefault();
            $("#response_<?php  echo $row['company_short_code_id'];?>").empty();
            $("#loader_<?php  echo $row['company_short_code_id'];?>").show();
            service_id = "<?php echo $row['service_id']; ?>";
            product_id = <?php echo $row['product_id']; ?>;
            short_code = "<?php echo $row['short_code']; ?>"
            var myData = 'service_id=' + service_id +  '&product_id='+product_id  +  '&short_code='+short_code; 
            //alert(myData);
	         jQuery.ajax({
                type: "POST", // Post / Get method
                url: "../process/getReceivedSms.php", //Where form data is sent on submission
                dataType:"text", // Data type, HTML, json etc.
                data:myData, //Form variables
                success:function(response){
                $("#response_<?php  echo $row['company_short_code_id'];?>").html(response);
