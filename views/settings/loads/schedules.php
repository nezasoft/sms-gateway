...
<?php
           ...
           $filter = 'LIMIT '.$val;
           $url = '?settings&action=levels&filter='.$val.'&';
           }else{
           $val = 50;
           $filter = '';
           $url = '?settings&action=levels&';
           }
		   $query = $conn->prepare("SELECT s.id AS schedule_id FROM schedules AS s LEFT JOIN frequency AS f ON f.id = s.frequency_id LEFT JOIN users AS us ON us.id = s.user_id 
		   WHERE s.company_id='".$_SESSION['FON_G_COMPANY_ID']."' ORDER BY s.id DESC");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT s.id AS schedule_id, s.start_date,s.end_date,s.active,s.schedule_time,s.create_date,us.fname,us.lname,s.schedule_name,f.frequency_name FROM schedules AS s LEFT JOIN frequency AS f ON f.id = s.frequency_id LEFT JOIN users AS us ON us.id = s.user_id 
		   WHERE s.company_id='".$_SESSION['FON_G_COMPANY_ID']."' ORDER BY s.id DESC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
        ...
        <table width="100%" class="table table-striped"  id="datatable-fixedcolumns" >
            ...
            <tr>  
				...
				<td><?php echo titleCase($row['fname'].' '.$row['lname']); ?></td>
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete PO Item
    url: '../process/edit_schedule.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[1, 'ScheduleName'],[2,'StartDate'],[3,'EndDate'],[4,'EndTime'],[8,'Active']]
    }
    , buttons: {
        edit: {
