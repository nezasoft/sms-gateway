...
<?php
		   ...
		   $query = $conn->prepare("SELECT id AS cat_id FROM category ORDER BY category_name ASC");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT id AS cat_id,category_name FROM category ORDER BY id ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
	 ...
	 <?php  
		...
		echo '<div class="alert alert-close alert-danger"><div class="alert-content"></div><font color="red"><i class="fa fa-check-circle"></i></font> There are currently no records!</div>';
         ...
         ?>
 <br/><br/><br/><br/><br/><br/><div id="rec_found" class="alert alert-success"><font color="green"><i class="glyph-icon icon-check-circle"></i></font>&nbsp;<?php    echo "Displaying <strong>".$count."</strong> records out of total <strong>".$num_rows."</strong> found ";?></div>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
	 jQuery.ajax({
                ...
                url: "../views/settings/loads/product_categories.php", //Where form data is sent on submission
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete PO Item
    url: '../process/edit_product_category.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[1, 'CategoryName']]
    }
    , buttons: {
        edit: {
