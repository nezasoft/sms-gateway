<?php
/**
 * @author Walter Omedo - Frontier Optical Networks Limited -- Suggest Group
 * 18th Dec 2018
 */
...
try{
	...
	$query = $conn->prepare("SELECT group_name, id AS group_id FROM groups WHERE company_id='".$_SESSION['FON_G_COMPANY_ID']."' AND (group_name LIKE '%".$keyword."%' ) ORDER BY group_name ASC LIMIT 30");
   $query->execute();   	
	...
	if($num_rows>=1){
      ...
      $count++;
		echo '<div class="alert alert-success alert-st-one" role="alert">
				   ...
				   <p class="message-mg-rt">Group Name: <strong>'.str_ireplace($keyword,$bold,titleCase($query_row["group_name"])).'</strong></p>
			  </div>';
		}
