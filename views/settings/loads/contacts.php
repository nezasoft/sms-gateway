...
<?php
           ...
           $filter = 'LIMIT '.$val;
           $url = '?settings&action=contacts&filter='.$val.'&';
           }else{
           $val = 50;
           $filter = '';
           $url = '?settings&action=contacts&';
           }
		   $query = $conn->prepare("SELECT c.id AS contact_id FROM contacts AS c 
		   								LEFT JOIN users as us ON us.id = c.user_id 
		   								WHERE c.company_id='".$_SESSION['FON_G_COMPANY_ID']."' ORDER BY c.contact_name,c.id ASC");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT c.id AS contact_id, c.contact_name,c.mobile_no,c.account_no,us.fname,c.date_created FROM contacts AS c 
			                         LEFT JOIN users as us ON us.id = c.user_id 
			                         WHERE c.company_id='".$_SESSION['FON_G_COMPANY_ID']."' 
			                         ORDER BY c.contact_name,c.id  ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
        ...
        <table width="100%" class="table table-striped"  id="datatable-fixedcolumns" >
         ...
         <th>Account No</th>
            ...
            <td><?php echo titleCase($row['contact_name']); ?></td>
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete PO Item
    url: '../process/edit_contact.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[1, 'ContactName'],[2, 'Mobile'],[3, 'AccountNo']]
    }
    , buttons: {
        edit: {
