...
<?php
           ...
           $filter = 'LIMIT '.$val;
           $url = '?settings&action=settings&filter='.$val.'&';
           }else{
           $val = 50;
           $filter = '';
           $url = '?settings&action=settings&';
           }
		   $query = $conn->prepare("SELECT ss.id AS sys_id FROM sys_settings AS ss LEFT JOIN users AS us ON us.id = ss.user_id LEFT JOIN company AS c ON c.id = ss.company_id ORDER BY ss.id ASC");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT ss.id AS sys_id, ss.section_type,ss.sys_password, ss.last_updated, c.company_name, us.fname, us.lname FROM sys_settings AS ss LEFT JOIN users AS us ON us.id = ss.user_id LEFT JOIN company AS c ON c.id = ss.company_id ORDER BY ss.id ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
        ...
        <table width="100%" class="table table-striped"  id="datatable-fixedcolumns" >
            ...
            <tr>  
				...
				<td><?php echo $row['sys_password']; ?></td>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
	 jQuery.ajax({
                ...
                url: "../views/settings/loads/settings.php", //Where form data is sent on submission
                dataType:"text", // Data type, HTML, json etc.
                data:myData, //Form variables
                success:function(response){
                $("#settings_data").html(response);
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete PO Item
    url: '../process/edit_settings.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[2, 'Password']]
    }
    , buttons: {
        edit: {
