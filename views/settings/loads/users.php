...
$where = "WHERE c.id LIKE '%%'";
}else{
$where = "WHERE c.id ='".$_SESSION['FON_G_COMPANY_ID']."'";
}
		   ...
		   $query = $conn->prepare("SELECT  us.id AS user_id FROM users AS us LEFT JOIN company AS c ON c.id = us.company_id LEFT JOIN levels as l ON l.id = us.level_id ".$where." ORDER BY us.fname ASC");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT  us.id AS user_id, us.fname, us.lname, us.email, DATE_FORMAT(us.date_created,'%d %c %Y') AS date_created, us.last_login, us.active, c.company_name, l.level_name, l.id AS level_id  FROM users AS us LEFT JOIN company AS c ON c.id = us.company_id LEFT JOIN levels as l ON l.id = us.level_id  ".$where." ORDER BY us.fname ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
    ...
    <tr>
      ...
      <th>Active</th> 
            ...
            ?>
      <tr>  
        <td><?php echo titleCase($row['user_id']); ?></td>
...
?>
 ...
 <?php
//Get Levl
$query = $conn->prepare("SELECT * FROM levels ORDER BY level_name ASC");
$query->execute();
$levels = $query->fetchAll(PDO::FETCH_ASSOC);
$level= "'{";
foreach($levels as $level_row){
 $level .= '"'.$level_row['id'].'":  "'.$level_row['level_name'].'",';
}
$level .='"'.$level_row['id'].'": "'.$level_row['level_name'].'"';                           
$level .="}'";
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete PO Item
    url: '../process/edit_user.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[1, 'Fname'],[2, 'Lname'],[3, 'Email'],[5, 'Level', <?php echo $level; ?>],[6, 'Active']]
    }
    , buttons: {
        edit: {
