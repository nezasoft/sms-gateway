...
<?php
		   ...
		   $query = $conn->prepare("SELECT cb.id AS assign_id FROM client_bulk_sms AS cb LEFT JOIN sms_package AS sp ON sp.id = cb.sms_package_id
                                    LEFT JOIN company AS c ON c.id = cb.client_id ORDER BY cb.id ASC ");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT cb.id AS assign_id, cb.amount, cb.balance, cb.activation_date, cb.expiry_date, sp.package_name, c.company_name FROM client_bulk_sms AS cb LEFT JOIN sms_package AS sp ON sp.id = cb.sms_package_id LEFT JOIN company AS c ON c.id = cb.client_id ORDER BY cb.id ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
        ...
        <table width="100%" class="table table-striped"  id="datatable-fixedcolumns" >
            ...
            <tr>  
				...
				<td><?php echo titleCase($row['package_name']); ?></td>
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete Group Item
    url: '../process/edit_client_package.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[3, 'SMSCount']]
    }
    , buttons: {
        edit: {
