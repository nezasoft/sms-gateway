...
<?php
		   ...
		   $query = $conn->prepare("SELECT id AS group_id FROM groups ORDER BY group_name ASC");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT id AS group_id,group_name FROM groups ORDER BY id ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
        ...
        <table width="100%" class="table table-striped"  id="datatable-fixedcolumns" >
            ...
            <tr>  
				...
				<td><?php echo titleCase($row['group_name']); ?></td>
                <td></td>
            </tr>
            <?php
            }
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete Group Item
    url: '../process/edit_group.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[1, 'GroupName']]
    }
    , buttons: {
        edit: {
