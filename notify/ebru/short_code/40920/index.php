...
//Lets get the message id
$get_message = $conn->prepare("SELECT id AS message_id, message FROM incoming_messages WHERE linkid='".$linkid."' AND company_id='".$company_id."' LIMIT 1");
$get_message->execute();
$get_message_row = $get_message->fetch(PDO::FETCH_ASSOC);
//Lets check if this record has been synced
$message_id = $get_message_row['message_id'];
$message = $get_message_row['message'];
