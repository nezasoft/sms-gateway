...
<?php
		   ...
		   $query = $conn->prepare("SELECT sc.id AS short_code_id FROM short_codes AS sc LEFT JOIN products AS p ON p.id = sc.product_id LEFT JOIN category AS c ON c.id = sc.category_id");
		   $query->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$query = $conn->prepare("SELECT sc.id AS short_code_id, sc.short_code, sc.short_code_description,sc.service_id,p.product_name,p.id AS product_id,  c.category_name,DATE_FORMAT(sc.date_created,'%d %c %Y') AS date_created,sc.status  FROM short_codes AS sc LEFT JOIN products AS p ON p.id = sc.product_id LEFT JOIN category AS c ON c.id = sc.category_id ORDER BY sc.id ASC LIMIT ".$pageLimit.",".$setLimit."");
			$query->execute();
      ...
      <th>Service ID</th>
            ...
            ?>
            <tr>  
            <td><?php echo titleCase($row['short_code_id']); ?></td>
            <td><?php echo $row['short_code']; ?></td>
				<td><?php echo $row['short_code_description']; ?></td>
...
//Get Products
$query = $conn->prepare("SELECT id,product_name FROM products ORDER BY product_name ASC");
$query->execute();
$products = $query->fetchAll(PDO::FETCH_ASSOC);
//Get Categories 
$query_2 = $conn->prepare("SELECT id, category_name FROM category ORDER BY category_name ASC");
$query_2->execute();
$categories =$query_2->fetchAll(PDO::FETCH_ASSOC);
$product= "'{";
foreach($products as $product_row){
 $product .= '"'.$product_row['id'].'":  "'.$product_row['product_name'].'",';
}
$product .='"'.$product_row['id'].'": "'.$product_row['product_name'].'"';                           
$product .="}'";
...
$('#datatable-fixedcolumns').Tabledit({
    //Delete PO Item
    url: '../process/edit_short_code.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'ItemID'], editable: [[1, 'ShortCode'],[2, 'Description'],[3, 'ServiceID'],[4, 'Product', <?php echo $product; ?>],[5, 'Category', <?php echo $category; ?>],[7, 'Status'],]
    }
    , buttons: {
        edit: {
