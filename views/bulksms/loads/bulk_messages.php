...
include("../../../connect/connect.php");
           ...
           $filter = 'LIMIT '.$val;
           $url = '?bulksms&action=view_bulk_sms_codes&filter='.$val.'&client_id='.$company_id.'&';
           }else{
           $company_id = $_SESSION['FON_G_COMPANY_ID'];
           $val = 10;
           $filter = '';
           $url = '?bulksms&action=view_bulk_sms_codes&client_id='.$company_id;
           }
		   $query = $conn->prepare("SELECT csc.id AS company_short_code_id FROM company_short_codes AS csc 
                                  ...
                                  WHERE csc.company_id='".$company_id."' AND sc.product_id='2'");
		   $query->execute();
		...
		return $setPaginate;
         ...
         $pageLimit  = ($page * $setLimit) - $setLimit;
			$query = $conn->prepare("SELECT csc.id AS company_short_code_id,csc.short_code_id,csc.response_message,csc.keyword,csc.endpoint,csc.correlator,csc.date_created,sc.short_code,sc.short_code_description,sc.service_id, p.product_name, c.company_name, us.fname, us.lname FROM company_short_codes AS csc 
                                   ...
                                   WHERE csc.company_id='".$company_id."' AND p.id='2'
        ...
        foreach($rows as $row){
          ...
          $('#other_content').empty();
 				  $('#page_content').empty();
              $("#wait").show();
              $('#page_content').load('../views/bulksms/messages.html?company='+company+'&short_code='+short_code+'&service_id='+service_id,function(responseTxt, statusTxt, jqXHR){
